...
function styleTransfer(sourceElementSelector, destinationElementSelector) {
  ...
  const destinationElements = document.getElementsByClassName(
...
function imageSearch(selector) {
  ...
  if (exploreButton) {
    ...
    exploreButton.addEventListener("click", () => {
      ...
      chrome.runtime
        .sendMessage({ action: "executeSearch", query: queryUrl })
        .then(() =>
          console.log("Successfully created new tab to display search results.")
        )
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === "insertion") {
    ...
    chrome.scripting
      ...
      .then(() => {
        console.log("Successfully executed button insertion.");
      })
      .catch((err) => console.log(err));
  }
});
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === "removal") {
    ...
    chrome.scripting
      ...
      .then(() => {
        console.log("Successfully executed element removal.");
      })
      .catch((err) => console.log(err));
  }
});
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === "tooltip") {
    ...
    chrome.scripting
      ...
      .then(() => {
        console.log("Successfully executed tooltip insertion.");
      })
      .catch((err) => console.log(err));
  }
});
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === "initialiseSearch") {
    ...
    chrome.scripting
      ...
      .then(() => {
        console.log("Successfully initialised image search instance.");
      })
      .catch((err) => console.log(err));
  }
});
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.action === "transfer") {
    ...
    chrome.scripting
      ...
      .then(() => {
        console.log("Successfully executed style transfer.");
      })
      .catch((err) => console.log(err));
  }
});
